...
1. **仕事を聞かれる。** 業務名・頻度・1回の時間・目的。`Artifact` が使える環境では1枚の入力シートが開き、1業務1行で書き込めます。
   ...
   ![棚卸しシート。業務名・誰が・回数と1回の時間・測ったか見積もりか・目的を1行ずつ書く。月の合計時間は自動で出る](docs/images/inventory-sheet.jpg)
...
図・設計・実行記録を重ねた HTML 1枚を `docs/harness/<名>/可視化/` に作ります。図のノードごとに、
何になったか・仕組み（指示・確認・柵・agent）・実装先のファイルと箇所・状態が1行で並び、図と表と実ファイルが相互にリンクします。

![可視化の1枚。第一階層の図の下に、ノードごとの実装先ファイルと仕組みの表が並ぶ](docs/images/harness-view-steps.jpg)

*このプラグイン自身の処理「業務を改善する」の1枚。評価レポートが無いので、状態はすべて「設計のみ」。*
