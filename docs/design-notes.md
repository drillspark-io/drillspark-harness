...
Background for the README. Nothing here is needed to install or run the plugin; it explains
why the plugin is shaped the way it is, and what has and has not been verified.
...
The plugin covers **two subjects with the same method**.
...
interviewing one question at a time; without it, the interview remains — ranks it by time using
ABC analysis *by machine* (the user is never asked to compute a cumulative percentage), draws the
one or two heaviest as a DrillSpark diagram, walks ECRS in order — **eliminate, combine, rearrange,
...
`skills/harness-visualize/` is a **処理 (workflow), not a 工程 (stage)** — it does not join the
chain below. It takes one workflow of a harness and renders the diagram, the design and what
actually happened into a single self-contained HTML page. It renders; it does not judge. Grading
stays with `harness-evaluator`. The model does not write the page: it writes a `map.json` (which
node became which file, mechanism and status; gates; pass conditions; excerpts) and copies the
DrillSpark diagrams into `diagrams.json`, and `scripts/harness-view-build.js` builds the page
deterministically and runs the guard before writing it.
...
as "unused"), and **how many times the harness stops its owner across the whole system**. It
assumes nothing is set up: `設計.md`, `.claude/rules/`, `.claude/tests/` and the diagrams may all
be absent, and creating that `設計.md` is often the most valuable thing a first improvement pass
leaves behind.
...
**Every stage of the generated harness leaves a file too.** On the diagram, each stage's output
is a document node with its output path in the note, and a deliverable that feeds the next stage
gets an inspection node in a separate agent lane (or the reason for not having one, recorded in
`図.md`). The generated skill checks that the previous stage's file exists before a stage starts.
In a real run without this, the deliverable was built first and its outline written last, and the
outline's author judged its own research.
...
`validate_diagram` checks syntax only. `scripts/diagram-lint.js` checks structure, deterministically:
...
### Process-improvement tables and plans
...
### How the tests are pinned

Every fixture under `tests/` carries an `expect: <CODE> x<count>` line (`%% expect:` in `.mmd`),
and the runner checks the reported code and count, not just the exit status. An exit code alone is
not a pass condition: a lint that returned 2 for everything would satisfy "violation sample exits
2". The one thing no fixture pins is the difference between the two `PRIVATE_INFO` scopes.

## Status and known limitations

Stated plainly, because a harness that overstates its own maturity is exactly the failure mode it
exists to prevent.

- **Verified only by its author, on the author's own work.** `process-improve` has been run on
  one person's real job list; `harness-implement` → `compose` → `evaluate` has built one harness
  (a landing-page workflow) that then ran one real task. No one outside the project has run it yet.
...
- The recurring shape: **the side that can only see one workflow is the side that catches the
  side that can see all of them.**
- **Measured against no harness at all, it has not yet shown a win.** The one comparison run so
  far was contaminated and is being re-taken; the honest reading today is *not proven*, not
  *proven ineffective*.
- **What did produce findings** is the separation: independent reviewers surfaced 24 MUST
  ...
  existed on the diagram but were missing from the implementation. The first real task run
  through a generated harness exposed three more defects the desk evaluation had missed.
...
- **Headless runs** (`claude -p`) have no `AskUserQuestion`, so approval gates fall back to a
  prose question and stop; and a project's own `CLAUDE.md` is loaded before any skill, so an
  instruction planted there runs before `harness-improve` can say it is material, not orders.
  `harness-visualize` did not finish when the model wrote the whole 60–120 KB page itself, which
  is why the page is generated from `map.json` by `scripts/harness-view-build.js`.
