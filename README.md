...
- **Each skill has been run once headless, in a sandbox** (`claude --plugin-dir … -p`, DrillSpark
  ...
  file excerpts, inline JS) in a single `Write` did not complete within 15–45 minutes. Until the
  page is generated by a script rather than written by the model, run that skill interactively
  and expect it to be slow. Two more things the sandbox showed: headless mode has no
